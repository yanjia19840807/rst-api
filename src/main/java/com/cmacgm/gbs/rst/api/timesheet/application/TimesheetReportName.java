 ...
 * <p>{@code Monthly Report of 202607(GBS INDIA).xlsx}
...
public final class TimesheetReportName {
    ...
    private static final Pattern DAILY = Pattern.compile(
            "^Daily Raw Data of (\\d{4}-\\d{2}-\\d{2}) - (.+?)(?:\\s+\\d+)?\\.(xlsx|csv)$",
            Pattern.CASE_INSENSITIVE);
    private static final Pattern MONTHLY = Pattern.compile(
            "^Monthly Report of (\\d{6})(?: (Revision(?: (\\d+))?))?\\((.+)\\)(?:\\s+\\d+)?\\.(xlsx|csv)$",
            Pattern.CASE_INSENSITIVE);
     ...
     * @param region configured Center name from the file name
